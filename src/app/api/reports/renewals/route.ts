/**
 * API Route Handler for /api/reports/renewals
 * GET: Retrieves renewal history report with optional filters.
 *
 * Query params: dateFrom, dateTo, affiliateId, instructorId
 *
 * Validates: Requirements 11.2, 11.8, 11.10, 12.6
 */
...
import { getRenewalHistory } from '@/services/report.service'
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await getRenewalHistory(parsed.data)
    ...
    return NextResponse.json(
      {
        ...
        data,
        ...(data.length === 0 && {
          message: 'No se encontraron registros de renovación para los filtros aplicados.',
        }),
      },
      { status: 200 }
    )
    ...
    return handleRouteError(error, {
      operation: 'GET /api/reports/renewals',
      method: 'GET',
      path: '/api/reports/renewals',
    })
  }
}
