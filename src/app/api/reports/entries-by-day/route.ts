/**
 * API Route Handler for /api/reports/entries-by-day
 * GET: Retrieves daily entry counts for a date range.
 *
 * Query params: dateFrom, dateTo
 * Max range: 90 days (enforced by service layer → returns 400)
 *
 * Validates: Requirements 11.6, 11.8, 11.10, 12.6
 */
...
import { getEntriesByDay } from '@/services/report.service'
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await getEntriesByDay(parsed.data.dateFrom, parsed.data.dateTo)
    ...
    return handleRouteError(error, {
      operation: 'GET /api/reports/entries-by-day',
      method: 'GET',
      path: '/api/reports/entries-by-day',
    })
  }
}
